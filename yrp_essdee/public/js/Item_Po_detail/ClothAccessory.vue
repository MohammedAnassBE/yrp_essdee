...
function createInput(attr, index, value, is_header){
    ...
    let el = root.value
    let is_read_only = cur_frm.cutting_attrs.includes(attr) || attr == 'Accessory' || attr == cur_frm.doc.stiching_major_attribute_value
    let df = {
        fieldtype: fieldtype,
        fieldname: attr+"_"+index,
        default: value,
        read_only: is_read_only,
        // v16's validate_link_and_fetch applies df.get_query filters server-side
        // and clears the value when it doesn't match the shared
        // set_packing_attr_map_value mapping. Skip validation for read-only
        // display columns so saved values render.
        ignore_link_validation: is_read_only,
    }
